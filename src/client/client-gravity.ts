...
function centreOfMass(balls: CANNON.Body[]): THREE.Vector3 {
    var totalMass = 0
    var totalX = 0
    var totalY = 0
    var totalZ = 0
    balls.forEach((ball) => {
        totalMass += ball.mass
        totalX += ball.mass * ball.position.x
        totalY += ball.mass * ball.position.y
        totalZ += ball.mass * ball.position.z
    })
...
for(var i=1; i<=numberOfCircles; i++) {
    ...
    rodBody.sleepTimeLimit = 30
    rodBody.sleepSpeedLimit = 0.001
...
circlesMoonsBodies.forEach((circlesMoonsBody, i) => {
    ...
    console.log(centreOfMass(circlesMoonsBodies.map(circlesMoonsBody => circlesMoonsBody.rodBody)))

    var com = centreOfMass(circlesMoonsBodies.map(circlesMoonsBody => circlesMoonsBody.rodBody)).normalize()
    console.log("Centre of mass normalised: " + com)
    var comQuaternion = new THREE.Quaternion();
    comQuaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), com)
    console.log("Centre of mass quaternion: ")
    console.log(comQuaternion)
    //centerSphereBody.quaternion.set(comQuaternion.x, comQuaternion.y, comQuaternion.z, comQuaternion.w)
